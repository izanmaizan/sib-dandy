...
$stats['pending'] = count(array_filter($bookings, function ($b) {
    return $b['status'] === 'pending';
}));
$stats['confirmed'] = count(array_filter($bookings, function ($b) {
    return $b['status'] === 'confirmed';
}));
$stats['completed'] = count(array_filter($bookings, function ($b) {
...
// Function untuk cek status return dress - TAMBAHAN BARU
function getDressReturnStatus($booking)
{
    ...
    // Cek status dari notes
    $dress_taken = strpos($notes, '[DRESS_TAKEN]') !== false;
    $dress_returned = strpos($notes, '[DRESS_RETURNED]') !== false;

    if ($dress_returned) {
        return [
            'status' => 'returned',
            'text' => 'Baju Dikembalikan',
            'class' => 'returned',
            'color' => '#28a745',
        ...
        <?php else: ?>
            <div class="bookings-grid">
                <?php foreach ($bookings as $booking): ?>
                    ...
                    $dress_return_status = getDressReturnStatus($booking); // TAMBAHKAN BARIS INI
                    ?>
                    <div class="booking-card">
                        ...
                        <div class="booking-body">
                            ...
                            <div class="status-item <?php echo $status_info['class']; ?>">
                                ...
                                <!-- Dress Return Status - DIPERBAIKI -->
                                <?php if ($dress_return_status): ?>
                                    <?php if ($dress_return_status['status'] === 'need_return'): ?>
                                        <!-- <a href="booking_detail.php?id=<?php echo $booking['id']; ?>#return" class="btn btn-sm"
                                            style="background: #dc3545; animation: pulse-button 2s infinite;">
                                            <i class="fas fa-undo"></i> Return Baju SEKARANG
                                        </a> -->

                                        <?php
                                        $remaining = $booking['total_amount'] - $booking['total_paid'];
                                        if ($remaining > 0): ?>
                                            <a href="payment.php?booking=<?php echo $booking['id']; ?>&return=1" class="btn btn-sm"
                                                style="background: #ff9800; color: white;">
                                                <i class="fas fa-money-bill"></i> Bayar & Kembalikan
                                                <small>(<?php echo formatRupiah($remaining); ?>)</small>
                                            </a>
                                        <?php endif; ?>

                                    <?php elseif ($dress_return_status['status'] === 'ready_take'): ?>
                                        <div class="btn btn-sm" style="background: #28a745; color: white; cursor: default;">
                            ...
                            <div class="booking-actions">
                                ...
                                <!-- Return Action Button - DIPERBAIKI -->
                                <?php if ($dress_return_status && $dress_return_status['status'] === 'need_return'): ?>
                                    <a href="booking_detail.php?id=<?php echo $booking['id']; ?>#return" class="btn btn-sm"
                                        style="background: #dc3545;">
                                        <i class="fas fa-undo"></i> Return Baju
                                    </a>
                                <?php endif; ?>
